...
const MOUEditor = () => {
    ...
    const downloadPDF = async (userName, userPhone) => {
        ...
        const allParagraphs = clone.querySelectorAll('p, div, span');
        allParagraphs.forEach(el => {
            // Only force justify if no specific alignment is set (like center)
            ...
            // IMPORTANT: Preserve explicit height values for schedules
            if (!el.style.height) {
                // Don't override existing height
            }
            if (el.tagName === 'SPAN') {
        ...
        // Specifically preserve and enforce schedule heights
        const scheduleElements = clone.querySelectorAll('[style*="height"]');
        scheduleElements.forEach(el => {
            if (el.style.height) {
                // Keep the explicit height and ensure it's respected
                const heightValue = el.style.height;
        ...
        const opt = {
            ...
            pagebreak: { mode: ['css', 'legacy', 'avoid-all'], before: 'div.pagebreak' },
    ...
    const downloadDOCX = async (userName, userPhone) => {
        ...
        // Ensure Schedule C and all content is properly formatted
        content = content.replace(/style="[^"]*minHeight[^"]*"/g, '');
        content = content.replace(/style="[^"]*overflow\s*:\s*visible[^"]*"/g, (match) => {
            return match.replace(/overflow\s*:\s*visible;?/g, '');
        });

        console.log('Content length:', content.length);

        try {
            // Wrap HTML in Word-compatible markup with proper formatting
            const docContent = `
                ...
                <head>
                    ...
                    <style>
                        ...
                        p { margin: 0 0 10pt 0; text-align: justify; page-break-inside: avoid; }
                        div { page-break-inside: avoid; }
                        ol, ul { margin: 10pt 0; padding-left: 20px; }
                        li { margin-bottom: 6pt; page-break-inside: avoid; }
                    </style>
                </head>
                <body>
                    ${content}
            ...
            <div className={`editor-preview ${mobileView !== 'preview' ? 'hidden-mobile' : ''}`}>
                ...
                <div className="preview-container" style={{ flex: 1, overflowY: 'auto', padding: '2rem', display: 'flex', justifyContent: 'center' }}>
                    <div id="mou-preview" className="preview-document" style={{
                        ...
                        <div style={{ textAlign: 'center', fontWeight: 'bold', marginBottom: '5px' }}>SCHEDULE 'A'</div>
                        <div style={{ textAlign: 'center', fontSize: '10pt', fontStyle: 'italic', marginBottom: '10px' }}>(Description of Entire Property)</div>
                        <div style={{
                            minHeight: '120px',
                            height: '120px',
                            textAlign: 'justify',
                            ...
                            {formData.property.scheduleA}
                        </div>

                        <div style={{ textAlign: 'center', fontWeight: 'bold', marginBottom: '5px' }}>SCHEDULE 'B'</div>
                        <div style={{ textAlign: 'center', fontSize: '10pt', fontStyle: 'italic', marginBottom: '10px' }}>(Undivided interest agreed to be sold in Schedule 'A' Property)</div>
                        <div style={{
                            minHeight: '80px',
                            height: '80px',
                            textAlign: 'justify',
                            ...
                            {formData.property.scheduleB}
                        </div>

                        <div style={{ textAlign: 'center', fontWeight: 'bold', marginBottom: '5px' }}>SCHEDULE 'C'</div>
                        <div style={{ textAlign: 'center', fontSize: '10pt', fontStyle: 'italic', marginBottom: '10px' }}>(Description of the Apartment)</div>
                        <div style={{
                            minHeight: '140px',
                            height: '140px',
                            textAlign: 'justify',
                            ...
                            <table style={{ width: '100%', marginBottom: '40px', borderCollapse: 'collapse' }}>
                                <tbody>
                                    <tr style={{ verticalAlign: 'top' }}>
                                        ...
                                        <td style={{ width: '50%', paddingLeft: '20px', textAlign: 'right', verticalAlign: 'bottom' }}>
